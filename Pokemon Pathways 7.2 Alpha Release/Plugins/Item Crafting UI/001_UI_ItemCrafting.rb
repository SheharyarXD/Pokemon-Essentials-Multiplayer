...
class ItemCraft_Scene
  ...
  def pbCraftItem(stock)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        pocket = item.pocket
        if pbConfirmMessage(_INTL("Would you like to make {1} {2}?",volume,itemname))
  ...
  def removeIngredients(index,volume)
    for i in 0...@stock[index][1].length/2
      ...
      $PokemonBag.pbDeleteItem(item,volume*cost)
    end
  end
  ...
  def refreshNumbers(index,volume)
    ...
    textpos = []
    textpos.push([_INTL("x{1}",volume),@w+26,@h+94,0,BASEDARK,SHADOWDARK])
    num.times do |i|
  ...
  def pbRedrawItem(index,volume)
    ...
    num = @stock[index][1].length/2
    textpos = [
    ["USE: Craft",4,Graphics.height-38,0,BASELIGHT,SHADOWLIGHT],
    ["ARROWS: Navigate",Graphics.width/2,Graphics.height-38,2,BASELIGHT,SHADOWLIGHT],
    ["BACK: Exit",Graphics.width-4,Graphics.height-38,1,BASELIGHT,SHADOWLIGHT]
    ]
    textpos.push([item.name,@w+98,@h,0,BASEDARK,SHADOWDARK])
    num.times do |i|
      ingredient = GameData::Item.get(@stock[index][1][2*i])
      quantity = @stock[index][1][2*i+1]
