...
class Pokemon
  ...
  def calc_stats
    ...
    if self.shiny? && $game_variables[19]>$game_variables[20] && !self.shadowPokemon?|| self.shadowPokemon? && $game_variables[20]>$game_variables[19] &&  !self.shiny?
      ...
      @attack = (stats[:ATTACK] * 1.1).floor 
	  @defense = (stats[:DEFENSE] * 1.1).floor 
      @spatk = (stats[:SPECIAL_ATTACK] *1.1).floor
	  @speed = (stats[:SPEED] * 1.1).floor 
      @spdef = (stats[:SPECIAL_DEFENSE] *1.1).floor
      ...
      @attack = (stats[:ATTACK] * 1.3).floor 
	  @defense =(stats[:DEFENSE] * 1.3).floor 
      @spatk   = (stats[:SPECIAL_ATTACK] *1.3).floor
	  @speed   = (stats[:SPEED] * 1.3).floor 
      @spdef   = (stats[:SPECIAL_DEFENSE] *1.3).floor
    end
