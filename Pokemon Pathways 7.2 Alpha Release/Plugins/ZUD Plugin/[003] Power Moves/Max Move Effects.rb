...
# G-Max Aurastorm, G-Max Soulshards, G-Max Aura Vortex, G-Max Soulburn.
#===============================================================================
...
class PokeBattle_Move_D002 < PokeBattle_MaxMove_StatUp
  def initialize(battle,move,newMove)
    ...
    @statUp = [:DEFENSE,1]          if @id == :GMAXSOULSHARDS
    @statUp = [:SPECIAL_DEFENSE,1]  if @id == :GMAXAURAVORTEX
    @statUp = [:ATTACK,1]           if @id == :GMAXSOULBURN
    @statUp = [:ATTACK,6]           if @id == :GMAXSOULCRUSH
    @statUp = [:SPEED,6]            if @id == :GMAXSOULREND
    @statUp = [:SPEED,6]            if @id == :GMAXSOULBREAK
    @statUp = [:ATTACK,6]           if @id == :GMAXSOULVORTEX
  end
end
